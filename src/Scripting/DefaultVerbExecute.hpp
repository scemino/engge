...
public:
  explicit SetDefaultVerb(Engine &engine) : _engine(engine) {}
...
private:
  void execute(const Verb *pVerb, Entity *pObject1, Entity *pObject2) override {
    ...
    if (needsToWalkTo(pVerb->id, pObjToWalkTo)) {
      auto walk = std::make_unique<ActorWalk>(_engine, *pActor, pObjToWalkTo, sentence.get(), pVerb);
    ...
    auto verb = std::make_unique<VerbExecuteFunction>(_engine, *pActor, pObject1, pObject2, pVerb);
    sentence->push_back(std::move(verb));
    auto setDefaultVerb = std::make_unique<SetDefaultVerb>(_engine);
