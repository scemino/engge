...
bool FntFont::loadFromFile(const std::string& path)
{
	...
	int dirCharPos = path.find_last_of("/\\");
	std::string dir = path.substr(0, dirCharPos + 1);
...
bool FntFont::parse(const std::string& path)
{
	...
	if (!ifs.good())
	{
		std::cerr << "ERROR: FntFont::loadFromFile: "
				  << "couldn't open file \"" << path << "\"" << std::endl;
		return false;
	}
...
const sf::Glyph &FntFont::getGlyph(sf::Uint32 codePoint, unsigned int characterSize, bool bold, float outlineThickness) const
{
